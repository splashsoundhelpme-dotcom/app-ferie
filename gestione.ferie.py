...
# --- 3. LOGICA DI MATURAZIONE MENSILE AUTOMATICA ---
def aggiorna_maturazioni_mensili(df_dip, config):
    ...
    if oggi.month != ultimo_mese_registrato:
        st.warning(f"Sincronizzazione saldi per il mese di {oggi.strftime('%B')}...")
...
# --- 4. INIZIALIZZAZIONE DATABASE (ELENCO COMPLETO) ---
def inizializza_database():
    ...
    if not os.path.exists(FILE_DIPENDENTI):
        ...
        # Aggiornamento strutturale per file esistenti
...
# --- 5. LOGICA CORE E INTERFACCIA ---
st.set_page_config(page_title="Battistolli HR Portal v47.0", layout="wide")
...
if user == "admin":
    ...
    t1, t2, t3 = st.tabs(["Registro O.D.S.", "Editor Dipendenti (Fleggabili)", "Impostazioni"])
    
    with t1:
        st.subheader("Elenco Richieste Totali")
    ...
    with t2:
        ...
        st.info("Fleggare 'Escluso' per chi non deve influire sul limite assenze (es. coordinatori).")
        df_edit = st.data_editor(
            df_dip[['Nome', 'Contratto', 'Escluso', 'Ferie', 'ROL']],
            column_config={"Escluso": st.column_config.CheckboxColumn("Escluso dai Limiti", default=False)},
            disabled=["Nome"], use_container_width=True
        )
    ...
    with t3:
        ...
        n_lim = st.slider("Limite assenze operative", 1, 15, int(config['limite']))
        if st.button("Salva Config"):
            salva_config(n_lim, config['ultimo_mese'])
            st.success("Parametri aggiornati!"); st.rerun()

# --- 7. AREA UTENTE ---
else:
    # Cambio Password Primo Accesso
    if st.session_state.get("primo_accesso", False):
        st.warning("🔒 Sicurezza: È necessario cambiare la password al primo accesso.")
        ...
        if st.button("AGGIORNA PASSWORD"):
            if p1 == p2 and len(p1) > 4:
                ...
                st.success("Password salvata!"); time.sleep(1); st.rerun()
    ...
    # --- FUNZIONE ANNULLA O.D.S. ---
    st.subheader("Le mie prenotazioni")
    mie_r = df_ferie[df_ferie['Nome'] == user]
    if mie_r.empty: st.info("Nessuna richiesta attiva.")
    for i, r in mie_r.iterrows():
        ca, cb = st.columns([4, 1])
        ca.info(f"📅 **{r['Tipo']}** dal {r['Inizio']} al {r['Fine']}")
        if cb.button("❌ Annulla", key=f"del_{i}"):
            df_ferie = df_ferie.drop(i)
            df_ferie.to_csv(FILE_FERIE, index=False)
            invia_email(f"CANCELLAZIONE: {user}", f"Utente {user} ha rimosso {r['Tipo']} dal {r['Inizio']} al {r['Fine']}")
            st.rerun()

    st.divider()
    
    # Logica Saturazione con Esclusi
    ...
    st.subheader(f"Situazione Reparto (Soglia Operativi: {config['limite']})")
    gg_v = pd.date_range(date.today() + timedelta(days=1), periods=10).date
    cols = st.columns(len(gg_v))
    for idx, g in enumerate(gg_v):
        n = check_limite(g)
        with cols[idx]:
    ...
    with st.form("new_request"):
        ...
        if st.form_submit_button("INVIA O.D.S."):
            giorni_r = pd.date_range(d_dal, d_al).date
            conflitto = False
            
            if not info['Escluso'] and tipo not in ["Permesso 104", "Congedo Parentale"]:
                if any(check_limite(d) >= config['limite'] for d in giorni_r):
                    conflitto = True
            
            if conflitto:
                st.error(f"Spiacente, limite di {config['limite']} operatori raggiunto in queste date.")
            else:
                nuovo = pd.DataFrame([[user, str(d_dal), str(d_al), tipo, tipo, len(giorni_r), "GG"]], columns=df_ferie.columns)
                nuovo.to_csv(FILE_FERIE, mode='a', header=False, index=False)
                invia_email(f"Nuova Richiesta {tipo}: {user}", f"{user} dal {d_dal} al {d_al}")
