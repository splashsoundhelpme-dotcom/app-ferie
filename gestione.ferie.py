...
# --- DATABASE REALE (SOLO I 45 NOMI DEL TUO FILE) ---
def popola_database_iniziale():
    if not os.path.exists(FILE_DIPENDENTI) or os.stat(FILE_DIPENDENTI).st_size == 0:
        dati = [
            ...
            ["MINECCIA GIUSEPPE", 66.6, 32.7], ["MOLINARO NATALIZIA", 104.5, 42.1],
            ["MONGELLI SAVINO", 88.3, 15.4], ["NICOLAIDIS GIOVANNI", 12.4, 5.0],
            ["ONESTO MARCO", 5.2, 0.0], ["ORLANDO GIUSY", 243.61, 93.9],
            ["PACE ROSA", 3.01, 10.15], ["PADOVANO MICHELE", 77.4, 22.1],
...
# --- LOGIN E LOGICA (INVARIATI) ---
inizializza_files()
df_dip = pd.read_csv(FILE_DIPENDENTI)

if "user" not in st.session_state:
    st.title("🏢 Accesso Battistolli HR")
    u_input = st.text_input("Inserisci NOME COGNOME (Maiuscolo)")
...
if st.session_state["user"] != "admin":
    ...
    st.write(f"Saldo Ferie 2025: **{dati_u['Saldo_Ferie_2025']} ore**")
    st.write(f"Saldo ROL 2025: **{dati_u['Saldo_ROL_2025']} ore**")
    # ... resto della logica form ...
else:
    st.title("Pannello Admin")
    st.dataframe(df_dip[['Nome', 'Saldo_Ferie_2025', 'Saldo_ROL_2025']])
