...
# --- DATABASE MANUALE ESTRATTO DAL TUO CSV (45 RIGHE) ---
def reset_database_forzato():
    dati = [
...
# Inizializzazione forzata (se i nomi non corrispondono o il file è vuoto)
if os.path.exists(FILE_DIPENDENTI):
    df_temp = pd.read_csv(FILE_DIPENDENTI)
    if "MOLINARO NATALIZIA" not in df_temp['Nome'].values:
        df_dip = reset_database_forzato()
    else:
        df_dip = df_temp
else:
    df_dip = reset_database_forzato()
    ...
    if st.button("ACCEDI"):
        if u_input == "ADMIN" and p_input == PASSWORD_ADMIN:
            ...
            if not user_row.empty:
                # Confronto password come stringa per sicurezza
                if p_input == str(user_row.iloc[0]['Password']).split('.')[0]:
...
if st.session_state["user"] != "admin":
    ...
    c1, c2 = st.columns(2)
    c1.metric("Saldo Ferie", f"{dati_u['Ferie']} h")
    c2.metric("Saldo ROL", f"{dati_u['ROL']} h")
    
    # Form per invio (omesso per brevità, ma già presente nelle versioni precedenti)
    st.info("Sistema pronto per l'inserimento richieste.")
else:
    st.title("Pannello Admin")
    st.write("### Database Completo")
